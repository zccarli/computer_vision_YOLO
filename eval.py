...
class Evaluation:
    ...
    def evaluate(self):
        ...
        for class_name in CAR_CLASSES:       # self.predictions is a dictionary that holds the predictions for each class. It is a dictionary with the key being the class name and the value being a list of predictions for that class.
            class_preds = self.predictions[class_name]  # class_preds [[image_id,confidence,x1,y1,x2,y2],...]
            ...
            BB = np.array([x[2:] for x in class_preds]) # Bounding Box [[x1,y1,x2,y2],...]
            # sort by confidence
            sorted_ind = np.argsort(-confidence) # descending order
            sorted_scores = np.sort(-confidence)
            BB = BB[sorted_ind, :] # BB rank by confidence 
            image_ids = [image_ids[x] for x in sorted_ind] # image_ids rank by confidence
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default="HKU-DASC7606-A1/checkpoints/hku_mmdetector_best.pth", help='Pretrained Model Path')
    parser.add_argument('--output_file', default="HKU-DASC7606-A1/outputs/result.pkl", help='PKL for evaluation')
